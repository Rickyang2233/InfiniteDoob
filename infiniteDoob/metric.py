...
def initialize(M,truncate_high_order_derivatives=False):
    ...
    ##### Sharp and flat map (used for calculating tangent vectors and covectors with the metric):
    M.flat = lambda x, v: jnp.tensordot(M.g(x), v, axes=(1, 0))          # calculate the covector with the given metric and tangent vector 
    M.sharp = lambda x, p: jnp.tensordot(M.gsharp(x), p, axes=(1, 0))    # calculate the tangent vector with the given metric and covector

    ##### Christoffel symbols
    # \Gamma^i_{kl}, indices in that order
    # \Gamma^i_{kl} := 1/2 * g^{km} * (dg_{ik}/dq^j + dg_{jk}/dq^i - dg_{ij}/dq^k)
    def Gamma_g(x):
        Dgx = M.Dg(x)
        gsharpx = M.gsharp(x)
    ...
    M.grad = lambda x, f: M.sharp(x, gradx(f)(x))                                                           # Riemannian gradient for scalar fields f
    M.div = lambda x, X: jnp.trace(jacfwdx(X)(x)) + 0.5 * jnp.dot(X(x), gradx(M.logAbsDet)(x))              # Riemannian divergence for vector fields X
    M.divsharp = lambda x, X: jnp.trace(jacfwdx(X)(x)) - 0.5 * jnp.dot(X(x), gradx(M.logAbsDetsharp)(x))    # Riemannian divergence for covector fields X
    M.Laplacian = lambda x, f: M.div(x, lambda x: M.grad(x,f))                                              # Laplacian-Beltrami operator for scalar fields f
