...
class EmbeddedManifold(Manifold):
    ...
    def __init__(self,F=None,dim=None,emb_dim=None,invF=None):
        ...
        if F is not None:
            self.F = F
            self.invF = invF
            self.JF = jacfwdx(self.F)               # Jacobian of the parameterization, R^d -> T_x M, i.e. the basis of R^d in the tangent space
            self.invJF = jacfwdx(self.invF)         # Inverse of the Jacobian of the parametrization, T_x M -> R^d, i.e. the representation of basis of the tangent space in R^d
    ...
    def plot_path(self, xs, vs=None, v_steps=None, i0=0, color='b', 
        ...
        N = len(xs)
        prevx = None
        # iterativly call self.plotx() for plotting each point in the path
    ...
    def plotx(self, x, u=None, v=None, v_steps=None, i=0, color='b', label='', marker='o',               
              color_intensity=1., linewidth=1., s=15., prevx=None, prevchart=None, last=True):
        ...
        if type(x) == type(()):
            # x contains both coordinates and chart center on manifolds
            Fx = self.F(x)
            chart = x[1]
        else: 
            # x only contains coordinates on manifold
            Fx = x
            chart = self.centered_chart(Fx)
            # The chart center is determined by the chidren manifold class
            x = (self.invF((Fx,chart)), chart)
            # Recalculate the coordinates of x in the new chart
