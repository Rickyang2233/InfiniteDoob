...
class landmarks(Manifold):
    ...
    def __init__(self,
        ...
        Args:
            N (int, optional): Number of landmarks. Defaults to 1.
            m (int, optional): Dimension of each landmark, the total dimension of manifold will be N*m. Defaults to 2.
            k_alpha (_type_, optional): Kernel scaling. Defaults to 1..
            k_sigma (_type_, optional): Kernel variance. Defaults to None.
            kernel (str, optional): Type of kernel used. Defaults to 'Gaussian'.
            order (int, optional): Order of Sobolev kernels. Defaults to 2.
    ...
    def plotx(self, x, u=None, color='b', color_intensity=1., linewidth=1., prevx=None, last=True, curve=False, markersize=None, arrowcolor='k', marker='o'):
        ...
        for j in range(NN):
            if last:
                if self.m == 2:
                    plt.scatter(x[j,0],x[j,1],color=color,s=markersize, marker=marker)
                elif self.m == 3:
                    ax.scatter(x[j,0],x[j,1],x[j,2],color=color,s=markersize if markersize else 50, marker=marker)
            else:
                try:
                    #if not self.std_basis:
